...
func presentScene<Action>(animated: Bool, scene: @escaping () -> Scene<Action>) -> Observable<Action> {
    Observable.using({ ScenePresentationHandler(scene: scene(), animated: animated) }, observableFactory: { $0.action })
}
...
///   - animated: Pass `true` to animate the presentation; otherwise, pass `false`.
///   - scene: A factory function for creating the Scene.
func finalPresentScene<Action>(animated: Bool, scene: @escaping () -> Scene<Action>) {
    _ = presentScene(animated: animated, scene: scene)
        .subscribe()
}
...
private final class ScenePresentationHandler<Action>: Disposable {
    ...
    init(scene: Scene<Action>, animated: Bool) {
        child = scene.controller
        action = scene.action
        isAnimated = animated
